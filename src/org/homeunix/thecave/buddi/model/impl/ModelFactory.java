...
public class ModelFactory {
	...
	public static BudgetCategoryType getBudgetCategoryType(String name) {
		if (budgetPeriodTypes == null) {
			...
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_MONTH.toString(),
					new BudgetCategoryTypeMonthly());
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_WEEK.toString(),
					new BudgetCategoryTypeWeekly());
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_SEMI_MONTH.toString(),
					new BudgetCategoryTypeSemiMonthly());
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_QUARTER.toString(),
					new BudgetCategoryTypeQuarterly());
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_SEMI_YEAR.toString(),
					new BudgetCategoryTypeSemiYearly());
			budgetPeriodTypes.put(BudgetCategoryTypes.BUDGET_CATEGORY_TYPE_YEAR.toString(),
	 ...
	 * @throws InvalidValueException
	 */
	public static BudgetCategory createBudgetCategory(String name, BudgetCategoryType type, boolean income)
			throws InvalidValueException {
	...
	private static PromptHandler promptHandler = new SwingPromptHandler();

	public static void setPromptHandler(PromptHandler handler) {
		promptHandler = handler;
	}

	public static class SwingPromptHandler implements PromptHandler {
		@Override
	...
	public static Document createDocument() throws ModelException {
		...
		if (getAutoSaveLocation(null).exists() && getAutoSaveLocation(null).canRead()) {
			Logger.getLogger(ModelFactory.class.getName())
					.info("Autosave file found; prompting user if we should use it or not");
			...
			if (promptHandler.askUser(
					...
					options,
					options[0]) == 0) { // The index of the Yes button.
				try {
					...
					Logger.getLogger(ModelFactory.class.getName())
							.warning("Error opening auto save file.  Continuing on to create normal file.");
				} catch (OperationCancelledException oce) {
					Logger.getLogger(ModelFactory.class.getName())
							.finest("User cancelled opening auto save file.  Continuing on to create normal file.");
				}
			} else {
				Logger.getLogger(ModelFactory.class.getName()).info(
						"User decided not to load AutoSave file.  It will be removed the next time this file is saved.");
			}
		}
		...
		for (BudgetIncomeDefaultKeys s : BudgetIncomeDefaultKeys.values()) {
			document.addBudgetCategory(
					ModelFactory.createBudgetCategory(s.toString(), new BudgetCategoryTypeMonthly(), true));
		}
		...
		if (errors != null)
			promptHandler.showMessage(errors);
	 ...
	 * legacy format, use ModelConverter to get a Bean object, and call the
	 * constructor which
	...
	public static Document createDocument(File file) throws DocumentLoadException, OperationCancelledException {
		...
		try {
			...
			while (true) {
				try {
					...
					if (o instanceof DocumentImpl) {
						...
						throw new IncorrectDocumentFormatException(
								"Could not find a DataModelBean object in the data file!");
					}
					is.close();
					...
					for (BudgetCategory bc : document.getBudgetCategories()) {
						...
						for (String key : ((BudgetCategoryImpl) bc).getAmounts().keySet()) {
							...
							if (splitKey.length == 2) {
								...
								String newDateKey = bc.getBudgetPeriodType().getName()
						...
						if (changed) {
							Logger.getLogger(ModelFactory.class.getName()).warning(
									"Your data file has been updated to address bug #1811038.  Included inline are the details of the changes:\n\n"
											+ "New Map:\n" + newAmountMap + "\n\n"
			...
			// If we get here, something is very wrong. Perhaps the platform does not
			// support
	...
	/**
	 * Attempts to import a data model from extracted XML file. Works with Buddi 3
	 * extracted format.
	 * 
	 * @param file File to load
	 * @throws DocumentLoadException
	 ...
	 * @throws InvalidValueException
	 */
	public static ScheduledTransaction createScheduledTransaction(String name, String message, Date startDate,
			Date endDate, String frequencyType, int scheduleDay, int scheduleWeek, int scheduleMonth,
			String description, long amount, Source from, Source to) throws InvalidValueException {
	 ...
	 * @throws InvalidValueException
	 */
	public static Transaction createTransaction(Date date, String description, long amount, Source from, Source to)
			throws InvalidValueException {
	 ...
	 * is null, return the Preferences directory and a filename of
	 * "AutosaveData.buddi3autosave";
	 * if the base file is not null, return the location of the data file, with the
	 * extension
	...
	public static File getAutoSaveLocation(File baseFile) {
		if (baseFile == null)
			...
			autoSaveLocationString = autoSaveLocationString.replaceAll(
					Const.DATA_FILE_EXTENSION + "$", "");
			autoSaveLocationString = autoSaveLocationString
